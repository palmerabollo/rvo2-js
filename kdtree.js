function KdTree() {
	...
	var buildAgentTreeRecursive = function(begin, end, node) {
		...
		if (end - begin > MAXLEAF_SIZE) {
			// no leaf node
			isVertical = agentTree[node].maxX - agentTree[node].minX > agentTree[node].maxY - agentTree[node].minY;
			splitValue = isVertical ? 0.5 * (agentTree[node].maxX + agentTree[node].minX) : 0.5 * (agentTree[node].maxY + agentTree[node].minY);
	...
	this.buildObstacleTree = function() {
		obstacles = Simulator.instance.obstacles;
			...
			
			{
                ...
                for (var j = 0; j < obstacles.length; ++j)
                {
                    ...
                    else
                    {
                        ...
                        splitpoint = obstacleJ1.point.plus( (obstacleJ2.point.minus(obstacleJ1.point)).scale(t) );
    ...
    var queryVisibilityRecursive = function(/** Vector2 */ q1, /** Vector2 */ q2, radius, /** ObstacleTreeNode */ node)
    {
        ...
        else
        {
            ...
            q1LeftOfI = RVOMath.leftOf(obstacle1.point, obstacle2.point, q1);
            q2LeftOfI = RVOMath.leftOf(obstacle1.point, obstacle2.point, q2);
            invLengthI = 1.0 / RVOMath.absSq(obstacle2.point.minus(obstacle1.point));
            ...
            {
                point1LeftOfQ = RVOMath.leftOf(q1, q2, obstacle1.point);
                point2LeftOfQ = RVOMath.leftOf(q1, q2, obstacle2.point);
                invLengthQ = 1.0 / RVOMath.absSq(q2.minus(q1));
