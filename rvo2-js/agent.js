function Agent() {  
    ...
    this.update = function()
    {
        ...
        this.position = this.position.plus(this.velocity.scale(Simulator.instance.timeStep));
    };

    this.linearProgram1 = function(/** Array */ lines, /** int */ lineNo, /** float */ radius, /** Vector2 */ optVelocity, /** bool */ directionOpt)
    {
        dotProduct = lines[lineNo].point.multiply(lines[lineNo].direction);
        discriminant = RVOMath.sqr(dotProduct) + RVOMath.sqr(radius) - RVOMath.absSq(lines[lineNo].point);
        ...
        {
            denominator = RVOMath.det(lines[lineNo].direction, lines[i].direction);
            numerator = RVOMath.det(lines[i].direction, lines[lineNo].point.minus(lines[i].point));
